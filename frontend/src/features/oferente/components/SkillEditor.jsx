...
export default function SkillEditor({ features = [], habilidadesActuales = [], onSave }) {
  ...
  function addSkill() {
    const firstAvailable =
        hojas.find(f => !items.some(item => Number(item.caracteristicaId) === Number(f.id))) ||
        hojas[0];

    if (firstAvailable) {
      setItems(list => [
        ...list,
        { caracteristicaId: Number(firstAvailable.id), nivel: 3 }
      ]);
    }
  }
  ...
  function updateSkill(i, values) {
    setItems(list =>
        list.map((item, idx) => idx === i ? { ...item, ...values } : item)
    );
  }
  ...
  async function save() {
    ...
    items.forEach(h => {
      if (h.caracteristicaId) {
        map.set(Number(h.caracteristicaId), {
          caracteristicaId: Number(h.caracteristicaId),
        ...
        <div className="editor-list">
          {items.map((d, i) => (
              <div className="inline-form skill-row" key={i}>
                <select
                    value={d.caracteristicaId}
                    onChange={e => updateSkill(i, { caracteristicaId: Number(e.target.value) })}
                >
                  {hojas.map(f => (
                ...
                <LevelSelect
                    value={d.nivel}
                    onChange={nivel => updateSkill(i, { nivel })}
                />

                <button type="button" className="btn ghost" onClick={() => removeSkill(i)}>
                  Quitar
                </button>
              </div>
          ))}
          ...
          {!items.length && (
              <p className="hint-box">
                Aún no tienes habilidades registradas. Agrega la primera habilidad para que las empresas puedan encontrarte.
              </p>
          )}
        </div>
